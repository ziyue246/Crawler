...
public class UrlReduplicationRemove {
	...
	private void iniUrlsBloomFilter() throws UnsupportedEncodingException {
		...
		double error = 0.01d;
		int m = new Double(Math.log(error) * n
				/ (Math.log(1d - Math.pow(Math.E, -0.6d)) * 0.6)).intValue();
	...
	private int getQueryList(Map<String, List<String>> map) {
		return Systemconfig.mysqlService.getAllMd5(Systemconfig.table, map);
	}
	//删除重复的url
	private void deleteUrl(List<String> url, String table) {
		if(url.size()>0)
			System.err.println(table+"表中的重复数据将被删除！");
		Systemconfig.mysqlService.deleteReduplicationUrls(url, table);
	}

	/**
	 * 判断新加入数据表中的一个url列表中的url重复与否，返回不包含重复url的url列表
	 * 2013.6修改
