...
public class Extractor {
	...
	public static String getHostUrl(WeiboDataProcessed weibodata){
		String url = weibodata.getAuthorUrl();
		Splider splider = new Splider();
	...
	public static Bloggers getBloggerinfo(String url, WeiboDataProcessed weibodata){
		...
		blog.setAuthorUrl(weibodata.getAuthorUrl());
		blog.setCategoryCode(Integer.parseInt(weibodata.getCategoryCode()));
	...
	private static void getblogAuthor(Bloggers blog,Node node){
		...
		NodeList imgnl = DomTree.commonList(imgxpath, node);
		String imgs = imgnl.item(0).getTextContent();
		blog.setAuthorImg(imgs);
	}
	...
	private static void getCertif(Bloggers blog,Node node){
		String xpath = BloggersXPath.certify;
		NodeList nl = DomTree.commonList(xpath, node);
		if(nl.getLength()==0){
			blog.setCertify("普通");
		...
		}else if(s.contains("W_icon icon_pf_club")){
			blog.setCertify("达人");
			//blog.setCategoryCode(2);
		}
	}
