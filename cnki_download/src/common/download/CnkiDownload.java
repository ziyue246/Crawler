...
import common.http.sub.HttpCnki;
import common.system.SystemCommon;
import common.util.DomTree;
...
public class CnkiDownload {
	...
	public void download(List<String> message) {
		...
		for (int i = 0; i < message.size(); i++) {
			...
			String searchUrl = "http://epub.cnki.net/kns/request/SearchHandler.ashx?action=&NaviCode=*&ua=1.21&PageName=ASP.brief_result_aspx&DbPrefix=SCDB&DbCatalog=%e4%b8%ad%e5%9b%bd%e5%ad%a6%e6%9c%af%e6%96%87%e7%8c%ae%e7%bd%91%e7%bb%9c%e5%87%ba%e7%89%88%e6%80%bb%e5%ba%93&ConfigFile=SCDB.xml&db_opt=CJFQ%2CCJFN%2CCDFD%2CCMFD%2CCPFD%2CIPFD%2CCCND%2CCCJD&base_special1=%25&magazine_special1=%25&txt_1_sel=SU"
			...
			String urlXpath = "//A[@class='fz14']/@href";
			...
			while (true) {
				...
				if (html.getRealUrl() != null) {
					...
					if (orignUrl.contains("docdownload.cnki.net")) {
						...
						DownloadPDF.downloadPDF(html, fileName);
						SystemCommon.printLog(title+" collect success!!!");
						break;
					}
				} else {
					break;
